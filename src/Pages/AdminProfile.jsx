...
const AdminProfile = () => {
  ...
  const normalizeUser = (u, details = {}) => {
    const name =
      u.name || u.Name || details.name || details.Name || u.username || "";
    const email = u.email || u.Email || details.email || details.Email || "";
    const ssn =
      u.ssn || u.Ssn || details.ssn || details.Ssn || details.id || u.id || "";
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      const listWithDisplay = list.map((u) => ({
        ...u,
        displayRole:
          getRoleString(u.rawRole) ||
          (typeof u.rawRole === "string" ? u.rawRole : ""),
      }));

      const hasRoleField = listWithDisplay.some(
        (u) => u.displayRole && u.displayRole !== ""
      );
      if (hasRoleField) {
        const adminsOnly = listWithDisplay.filter((u) =>
          (u.displayRole || "").toLowerCase().includes("admin")
        );
  ...
  const handleCreateAdmin = async () => {
    ...
    if (!createForm.username || !createForm.email || !createForm.password) {
      return showAlert("Username, email and password are required", "error");
    }
    if (!createForm.phoneNumber) {
      return showAlert("Phone number is required", "error");
    }
    ...
    try {
      ...
      showAlert("Create admin failed: " + errorMsg, "error");
    }
  };
  ...
  const viewProfile = async (u) => {
    try {
      ...
      if (!ssnValue) {
        showAlert("User SSN not found", "error");
        return;
      }
  ...
  const getRoleString = (role) => {
    if (role === undefined || role === null || (role === "" && role !== 0))
      return "";
  ...
  const handleAddDisability = async () => {
    if (!disabilityForm.name)
      return showAlert("Disability name is required", "error");
    try {
  ...
  const handleDeleteUser = async (u) => {
    const ssnValue =
      u?.ssn || u?.Ssn || u?.SSN || u?.id || u?.Id || u?.username;
    if (!ssnValue) {
      showAlert("User SSN not found; cannot delete", "error");
      return;
    }
    const currentSsn =
      user?.ssn ||
  ...
  return (
    <div className="admin-page">
      <div className="page-container">
        ...
        <section
          className="card-section card-section--password"
          style={{ marginTop: 18 }}
        >
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  showAlert("Change password failed: " + errorMsg, "error");
                }
              }}
            >
              Change
        ...
        <section
          className="card-section card-section--create"
          style={{ marginTop: 18 }}
        >
          <h3>Create Admin</h3>
          <div className="card-row">
            ...
            <input
              ...
              onChange={(e) =>
                setCreateForm({ ...createForm, confirm: e.target.value })
              }
            />
          ...
          <div className="small-note">
            * Required fields. Phone: (e.g., +1234567890 or 01234567890)
          </div>
        ...
        <section className="card-section" style={{ marginTop: 18 }}>
          ...
          <div className="card-row">
            <input
              ...
              onChange={(e) =>
                setDisabilityForm({ ...disabilityForm, name: e.target.value })
              }
            />
            <input
              className="input"
              ...
              onChange={(e) =>
                setDisabilityForm({ ...disabilityForm, type: e.target.value })
              }
            />
            <input
              className="input"
          ...
          {disabilityMessage && (
            <p className="status-msg success">{disabilityMessage}</p>
          )}
        </section>
            ...
            <div className="profile-cards-grid">
              {admins.map((u, idx) => (
                ...
                >
                  ...
                  <div
                    style={{ color: "#6b7280", fontSize: 13, marginBottom: 6 }}
                  >
              ...
              {viewUser ? (
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ marginBottom: 8 }}>
                    ...
                    {viewUser.username || viewUser.Username || "-"}
                  </div>
                  <div style={{ marginBottom: 8 }}>
                    <strong>Role:</strong>{" "}
                    {displayRole(viewUser.role || viewUser.Role)}
                  </div>
                  <div style={{ marginBottom: 8 }}>
                    <strong>SSN:</strong>{" "}
                    {viewUser.ssn || viewUser.Ssn || viewUser.SSN || "-"}
                  </div>
                </div>
              ) : (
                <div>Loading...</div>
